                ...
                <ul class="ml-auto navbar-nav header-menu-action align-items-end">
                    <li class="nav-item dropdown menu-area">
                        <a href="{{route('cart.index')}}" class="nav-link">
                            <?php
                            if (Cart::instance('default')->content()->count() == 0 && auth()->check()) {
                                Cart::merge(auth()->id());
                            }
                            ?>
                    ...
                    @auth
                        @php
                            ...
                            $notifications = Auth::user()->notifications()->whereBetween('status', [0,1])
                                ->orderBy('notifications.updated_at', 'desc')->limit(30)->get();
                            ...
                            foreach ($message_notifications as $message_notification) {
                                $unseen_messages_count += $message_notification->cnt;
                            }
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu half-menu dropdown-menu-end" aria-labelledby="navbarDropdown"
                                ...
                                <div class="dropdown-header pb-2 mb-2 border-bottom">
                                    <span class="dropdown-title">Messages ({{ $unseen_messages_count }})</span>
                                </div>
                                <div class="content-max-scroll">
                                @foreach ($message_notifications as $message_notification)
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <img src="{{user_name(Auth::user()->id)[0]->uploads->getImageOptimizedFullName(100,100)}}" alt="avatar" class="w-30px rounded img-fluid border">
                            </a>
                            <ul class="dropdown-menu small-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                ...
                                @if (auth()->user()->role == 2)
                                    <div class="seperated-menu d-flex py-2 border-top">
                                        <ul>
                                            <div class="seller-menu-title text-uppercase fs-14 py-2 fw-700 w-100">Seller</div>
                                            ...
                                            <li><a class="dropdown-item" href="{{ route('seller.withdraw.get') }}">Withdraw Earnings</a></li>
                                ...
                                <div class="seperated-menu d-flex py-2 border-top">
                                    <ul>
                                        <div class="seller-menu-title text-uppercase fs-14 py-2 fw-700 w-100">General User</div>
                                        <li><a class="dropdown-item" href="{{route('dashboard')}}">Dashboard</a></li>
                                        <li>
                                            <a class="dropdown-item"
                                               href="{{route('orders.index')}}">{{ auth()->user()->role ? 'All Orders' : 'My Orders' }}</a>
                                        </li>
                                        <!--<li><a class="dropdown-item" href="{{route('wishlist')}}">My Favorites</a></li>-->
                                    </ul>
                                </div>
